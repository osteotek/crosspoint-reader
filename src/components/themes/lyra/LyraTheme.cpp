...
void LyraTheme::drawBatteryLeft(GfxRenderer& renderer, Rect rect, const bool showPercentage) const {
...
void LyraTheme::drawBatteryRight(GfxRenderer& renderer, Rect rect, const bool showPercentage) const {
...
void LyraTheme::drawHeader(GfxRenderer& renderer, Rect rect, const char* title) const {
  ...
  if (title) {
    auto truncatedTitle = renderer.truncatedText(
        UI_12_FONT_ID, title, rect.width - LyraMetrics::values.contentSidePadding * 2, CrossPointFont::BOLD);
...
void LyraTheme::drawTabBar(GfxRenderer& renderer, Rect rect, const std::vector<TabInfo>& tabs,
                           bool selected) const {
  ...
  for (const auto& tab : tabs) {
    const int textWidth = renderer.getTextWidth(UI_10_FONT_ID, tab.label, CrossPointFont::REGULAR);
...
void LyraTheme::drawList(GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
...
void LyraTheme::drawSideButtonHints(GfxRenderer& renderer, const char* topBtn, const char* bottomBtn) const {
...
Rect LyraTheme::drawPopup(GfxRenderer& renderer, const char* message) const {
  constexpr int margin = 15;
  constexpr int y = 60;
  const int textWidth = renderer.getTextWidth(UI_12_FONT_ID, message, CrossPointFont::REGULAR);
  ...
  renderer.drawText(UI_12_FONT_ID, textX, textY, message, true, CrossPointFont::REGULAR);
