...
void EpubReaderActivity::render(Activity::RenderLock&& lock) {
  ...
  if (section->pageCount == 0) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, 300, tr(STR_EMPTY_CHAPTER), true, CrossPointFont::BOLD);
  ...
  if (section->currentPage < 0 || section->currentPage >= section->pageCount) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, 300, tr(STR_OUT_OF_BOUNDS), true, CrossPointFont::BOLD);
    renderStatusBar(orientedMarginRight, orientedMarginBottom, orientedMarginLeft);
    renderer.displayBuffer();
    return;
  }
