...
void EpubReaderPercentSelectionActivity::render(Activity::RenderLock&&) {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, tr(STR_GO_TO_PERCENT), true, CrossPointFont::BOLD);

  const std::string percentText = std::to_string(percent) + "%";
  renderer.drawCenteredText(UI_12_FONT_ID, 90, percentText.c_str(), true, CrossPointFont::BOLD);
