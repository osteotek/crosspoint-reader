...
void XtcReaderActivity::render(Activity::RenderLock&&) {
  ...
  if (currentPage >= xtc->getPageCount()) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, 300, tr(STR_END_OF_BOOK), true, CrossPointFont::BOLD);
...
void XtcReaderActivity::renderPage() {
  ...
  if (!pageBuffer) {
    ...
    renderer.clearScreen();
    renderer.drawCenteredText(UI_12_FONT_ID, 300, tr(STR_MEMORY_ERROR), true, CrossPointFont::BOLD);
    renderer.displayBuffer();
    return;
  }
  ...
  if (bytesRead == 0) {
    ...
    free(pageBuffer);
    renderer.clearScreen();
    renderer.drawCenteredText(UI_12_FONT_ID, 300, tr(STR_PAGE_LOAD_ERROR), true, CrossPointFont::BOLD);
    renderer.displayBuffer();
    return;
  }
