...
void XtcReaderChapterSelectionActivity::render(Activity::RenderLock&&) {
  ...
  const int titleX =
      contentX + (contentWidth - renderer.getTextWidth(UI_12_FONT_ID, tr(STR_SELECT_CHAPTER), CrossPointFont::BOLD)) / 2;
  renderer.drawText(UI_12_FONT_ID, titleX, 15 + contentY, tr(STR_SELECT_CHAPTER), true, CrossPointFont::BOLD);
