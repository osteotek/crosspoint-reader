...
void CalibreConnectActivity::render(Activity::RenderLock&&) {
  ...
  if (state == CalibreConnectState::SERVER_STARTING) {
    renderer.drawCenteredText(UI_12_FONT_ID, pageHeight / 2 - 20, tr(STR_CALIBRE_STARTING), true, CrossPointFont::BOLD);
  } else if (state == CalibreConnectState::ERROR) {
    renderer.drawCenteredText(UI_12_FONT_ID, pageHeight / 2 - 20, tr(STR_CONNECTION_FAILED), true, CrossPointFont::BOLD);
  }
  renderer.displayBuffer();
}
...
void CalibreConnectActivity::renderServerRunning() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, tr(STR_CALIBRE_WIRELESS), true, CrossPointFont::BOLD);

  int y = 55 + TOP_PADDING;
  renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_WIFI_NETWORKS), true, CrossPointFont::BOLD);
  y += LINE_SPACING;
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_CALIBRE_SETUP), true, CrossPointFont::BOLD);
  y += LINE_SPACING;
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_CALIBRE_STATUS), true, CrossPointFont::BOLD);
  y += LINE_SPACING;
