...
void CrossPointWebServerActivity::render(Activity::RenderLock&&) {
  ...
  if (state == WebServerActivityState::SERVER_RUNNING) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, pageHeight / 2 - 20, tr(STR_STARTING_HOTSPOT), true, CrossPointFont::BOLD);
    renderer.displayBuffer();
  }
}
...
void CrossPointWebServerActivity::renderServerRunning() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, tr(STR_FILE_TRANSFER), true, CrossPointFont::BOLD);

  if (isApMode) {
    // AP mode display - center the content block
    int startY = 55;

    renderer.drawCenteredText(UI_10_FONT_ID, startY, tr(STR_HOTSPOT_MODE), true, CrossPointFont::BOLD);
