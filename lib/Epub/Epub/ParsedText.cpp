...
void ParsedText::addWord(std::string word, const CrossPointFont::Style fontStyle, const bool underline,
                         const bool attachToPrevious) {
  if (word.empty()) return;

  words.push_back(std::move(word));
  CrossPointFont::Style combinedStyle = fontStyle;
  if (underline) {
    combinedStyle = static_cast<CrossPointFont::Style>(combinedStyle | CrossPointFont::UNDERLINE);
  }
